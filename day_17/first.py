...
FILE='test.txt' # sol: 45
FILE='input.txt' # sol: 4005
...
best, best_x, best_y = None, 0, 0
for x_velocity in range(0, x2):
    for y_velocity in range(0, abs(y1)):
      max_y = function(x1, x2, y1, y2, x_velocity, y_velocity)
      if max_y != None and (best == None or best < max_y):
          #print(f'Found best y: {max_y}, init: [{x_velocity},{y_velocity}]')
          best = max_y
          best_x = x_velocity
          best_y = y_velocity

print(f'result top y: {best}, init: [{x_velocity},{y_velocity}]')
